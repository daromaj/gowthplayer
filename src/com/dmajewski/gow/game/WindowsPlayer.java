...
//			Color numColor = ScreenProcessor.averageColor(bufferedImage, (int) Math.round(1040 * xImgFix),
//					(int) Math.round(15 * yImgFix), (int) Math.round(70 * xImgFix), (int) Math.round(35 * yImgFix));
			// avgNumber = numColor;
...
//			ImageIO.write(roundNumber, "png", Files.createTempFile("num", ".png").toFile());
			...
			if (endGame < 2 && !bufferedImagesEqual(roundNumber, prevRoundNumber)) {
				...
				if(initialState.getBoard()[0][0] == 9){
					endGame++;
				}else{
					try {
						endGame = 0;
...
//						e.printStackTrace();					
			...
			if(exceptionCount > 10){
				...
				String message = "I can't regonize gem at: " + pos[0] + "," + pos[1] + " color: " + Arrays.toString(initialState.getBoardColors()[pos[0]][pos[1]]);
				info(message);
				...
				for (int dir = 0; dir < 2; dir++) {
					if (dir == 0) {// right
						if (j + 1 > 7 || s.getBoard()[i][j] == s.getBoard()[i][j + 1]
								|| s.getBoard()[i][j] > 6 || s.getBoard()[i][j + 1] > 6) {
							continue;
						}
						State tmp = s.clone();
						...
						if (i + 1 > 7 || s.getBoard()[i][j] == s.getBoard()[i + 1][j]
								|| s.getBoard()[i][j] > 6 || s.getBoard()[i + 1][j] > 6) {
							continue;
						}
						State tmp = s.clone();
						int tmpType = tmp.getBoard()[i][j];
